...
w('"price": str(h.price) if h.price else (str(available_room.price_per_semester) if available_room else "0"),')
...
w('"capacity": r.capacity, "available_quantity": r.available_quantity, "price_per_semester": str(r.price_per_semester),')
...
w('cover = hostel.images.filter(is_cover=True).first()')
w('rooms_data = [{"id": str(r.id), "room_number": r.room_number, "room_name": r.room_name, "room_type": r.room_type, "capacity": r.capacity, "price_per_semester": str(r.price_per_semester), "description": r.description, "private_bathroom": r.private_bathroom, "air_conditioning": r.air_conditioning, "wifi": r.wifi, "status": r.status} for r in rooms]')
...
w('students = int(data.get("students", 1))')
w('semesters = (check_out - check_in).days')
w('if semesters <= 0: return JsonResponse({"error": "Invalid date range"}, status=400)')
w('total_price = room.price_per_semester * semesters')
w('booking = Booking.objects.create(hostel_id=room.hostel_id, room=room, customer_id=request.user.id, check_in=check_in, check_out=check_out, students=students, semesters=semesters, total_price=total_price, special_requests=data.get("special_requests", ""), booking_status="Pending", payment_status="Pending")')
