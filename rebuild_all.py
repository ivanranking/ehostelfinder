#!/usr/bin/env python3
"""Completely rebuild views.py from scratch with correct indentation."""
import ast, os

os.chdir('c:/Users/user/ehostelfinder')

# Build the complete views.py
...
def hostel_detail(request, id):
    try:
        ...
        rooms_data = [{
            ...
            "description": r.description, "size_sq_meters": str(r.size_sq_meters) if r.size_sq_meters else None,
...
def create_booking(request):
    ...
    try:
        ...
        return JsonResponse({
            ...
            "hostel_id": str(booking.hostel_id), "room_id": str(booking.room_id),
            ...
            "payment_status": booking.payment_status, "special_requests": booking.special_requests,
        }, status=201)
    except Exception as e:
...
def create_message(request):
    try:
        ...
        return JsonResponse({
            "id": str(msg.id), "hostel_id": str(msg.hostel_id),
            "full_name": msg.full_name, "email": msg.email, "subject": msg.subject,
            "message": msg.content, "created_at": msg.created_at.isoformat(),
        }, status=201)
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500)
...
def ai_chat(request):
    try:
        ...
        return JsonResponse({"candidates": [{"content": {"parts": [{"text": ai_response}]}}]})
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500)
    ...
    try:
        ...
        try:
            ...
            user = User.objects.create(
                ...
                last_name=last_name, provider="google",
            )
        auth_login(request, user)
        return redirect("home")
    except Exception as e:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for index, (room_type, price, is_avail) in enumerate(room_types, start=1):
                ...
                capacity = capacity_map.get(room_type, 1)
                Room.objects.create(
                    hostel=hostel, room_number=str(index), room_name=f"{room_type} Room",
...
def manager_checkins(request):
    ...
    if request.method == "GET":
        ...
        return JsonResponse(checkins_data, safe=False)
...
def admin_remove_manager(request):
    ...
    try:
        ...
        if not manager_id:
            return JsonResponse({"error": "manager_id required"}, status=400)
        manager_profile = get_object_or_404(Profile, id=manager_id, role="manager")
...
def admin_create_manager(request):
    ...
    try:
        ...
        if not full_name or not email or not password:
            return JsonResponse({"error": "Full name, email, and password required"}, status=400)
        if len(password) < 8:
...
def api_unassigned_managers(request):
    ...
    managers_data = [{"id": str(m.id), "full_name": m.full_name, "email": m.email} for m in managers]
    return JsonResponse(managers_data, safe=False)
...
def process_payment(request):
    try:
        ...
        payment = Payment.objects.create(
            ...
            transaction_reference=data.get("transaction_reference", f"TXN-{uuid.uuid4().hex[:12].upper()}"),
        ...
        Notification.objects.create(
            ...
            message=f"Payment of UGX {booking.total_price} for booking {booking.booking_reference} completed.",
        )
        return JsonResponse({
...
def cancel_booking(request, booking_id):
    try:
        ...
        if booking.booking_status in ["Checked Out", "CancelledThe edit corrupted the file. Let me write a fresh, clean script:

<create_file>
<absolute_path>c:/Users/user/ehostelfinder/gen_final_views.py</absolute_path>
<content>#!/usr/bin/env python3
"""Generate complete views.py with proper indentation."""
import ast, os

os.chdir('c:/Users/user/ehostelfinder')

# Read the current (corrupted) views.py that has most functions but wrong indentation
with open('hostels/views.py', 'r', encoding='utf-8') as f:
    content = f.read()

# The issue is that gen_views_all.py produced wrong indentation after the signup function.
# Let me find and fix every common indentation pattern.

# Strategy 1: The core issue is that after 'except: pass' on its own line, 
# the following lines lost indentation. Also, the else clause.
...
for line in lines:
    ...
    if ls.startswith('def '):
        fname = ls.split('(')[0].replace('def ', '').strip()
        current_funcs.append(fname)

print(f"Functions found: {len(current_funcs)}")
for fn in current_funcs:
    print(f"  - {fn}")
...
missing = [fn for fn in required if fn not in current_funcs]
